...
namespace llvm {
  ...
  void dataFlow::applyTransferFunction(struct basicBlockProps* basicBlock)
  {
    ...
    std::map<int,std::vector<int>>::iterator it = basicBlock->checkLHS.begin();
    while(it != basicBlock->checkLHS.end())
    {
      if(killSet[it->first])
      {
        std::vector<int>LHS = it->second;
        for(int i : LHS)
        {
          if(basicBlock->bbOutput[i])
          {
            killSet.reset(it->first);
          }
          else
          {
           killSet.set(it->first);
          }
        }
      }
      it++;
